...
Актуальная структура верхнего меню:

- `Калькулятор`
- `Отслеживание`
- `Услуги` - выпадающее меню:
  - `Доставка из интернет-магазинов РФ`
  - `Отправления в РФ`
  - `Сборные грузы (LTL)`
  - `Полная загрузка (FTL)`
- `Бизнесу`
...
На главной странице отдельный блок `Наши услуги:` удалён. Услуги теперь доступны через выпадающее меню.
...
В `Что не входит по умолчанию` нет пункта про грузчиков. Пункт `Простой сверх нормы - по договору` остаётся.
...
В LTL блок `Образец КП` вынесен в отдельную тёмно-синюю секцию после сравнения. Из сравнения и `Что не входит по умолчанию` убраны грузчики. В `Что не входит по умолчанию` добавлен пункт `Простой сверх нормы - по договору`.
...
После последних правок меню и страниц услуг повторно проверено:

- `npm run build -w frontend` в `C:\Users\Dell\Documents\Codex\sarmaexpress-main`
- `npm run build -w frontend` в `C:\Users\Dell\Documents\Codex\sarmaexpress-pr-work`
